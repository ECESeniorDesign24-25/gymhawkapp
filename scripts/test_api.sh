...
case "$FUNCTION" in
    ...
    getDeviceState)
        ...
        URL="https://gymhawk-2ed7f.web.app/api/$FUNCTION?thing_id=$THING_ID&variable=$VARIABLE"
        ;;
    getPeakHours)
        ...
        URL="https://gymhawk-2ed7f.web.app/api/$FUNCTION?thing_id=$THING_ID&date=$DATE&start_time=$START_TIME&end_time=$END_TIME"
        if [ ! -z "$PEAK" ]; then
            URL="$URL&peak=$PEAK"
        fi
        ;;
